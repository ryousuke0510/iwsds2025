...
def negotiate():
    ...
    with open('negotiate_data.pkl','rb') as infile,\
        ...
        for sample in data:
            ...
            for turn, act in zip(sample['raw_data']['events'], sample['dial_acts'][1:]):
                ...
                nego_one_data = {"item_description":sample['scene'], "traget_price":selling_price, "buyer_price":target_price, "gold_respose":target['response'], "dialogue_history":', '.join(conv)}
                nego_data.append(nego_one_data)
    ...
    # JSONL形式でファイルに書き戻す
    with open("negotiate_data.json", 'w', encoding='utf-8') as file:
        json.dump(nego_data, file, ensure_ascii=False, indent=4)
